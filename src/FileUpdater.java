...
public class FileUpdater implements Runnable {
	...
	try{
	    ...
	    if (debug) {
		debugger.update(" --- ACL not updated, shutting down server");
	    }
	    return false;
	}
	return true;
	...
	while (this.getRun()) {
		...
		if ((adjustedRemoteDate) > beacon.getFileDate()) {
			...
			if (debug) debugger.update(" ---- Receive failed...sending out another beacon to reestablish");
			beacon.sendMessage();
		    }
		}
		if ((adjustedRemoteDate) < beacon.getFileDate()) { 
			...
			debugger.update(" --- Address is" + remoteAddress.toString());
		    }
		...
		if (debug) debugger.update (" ---- " + remoteAddress.getHostName() +  " is NOT in the List");
	    }
