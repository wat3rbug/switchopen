...
public class SwitchOpen {
	...
	*/
	...
	if (debug) 
	    debugger.update("laf " + UIManager.getLookAndFeel().getName());
    ...
    public static void main(String[] args) {
		...
		} else debugger.update(" --- Server not started ---");
    }
	...
	if (switches.size() < 1 ) {
	    JOptionPane.showMessageDialog(frame, 
                 "Import a file because you have no data", "No switch data", 
					  JOptionPane.ERROR_MESSAGE);
	}
	for (int i = 0; i < switches.size(); i ++) {
		...
		// string from array
		if (debug) {
		    debugger.update("Found " + switches.get(i));
		}
		try {
		    String destination ="";
	...
	public void actionPerformed(ActionEvent es) {
		...
		// reads contens from selected file and puts them in new file
